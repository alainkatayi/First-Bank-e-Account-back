...
from .views import RequestCreatedView, RequestListView, TakeDecisionView, DecisionListView, RequestDetailView

urlpatterns = [
    path('store', RequestCreatedView.as_view(), name='soumission'),
    path('index', RequestListView.as_view(), name='get-all-request'),
    path('<int:id>', RequestDetailView.as_view(), name='get-one-request'),
